...
class StarBadgeCounter:
    ...
    def on_penalty(self, active_play_time_secs: int, state: Optional[str], difficulty, badges_locked_on_board: list[str]) -> Tuple[Optional[str], Optional[str]]:
        ...
        if state is None:
            raise Exception("should never happen, on_game_started is called above")
    ...
    def _find_smallest_star(self, badges_locked_on_board: list[str]):
        for candidate in ["s0", "s1", "s2"]:
            if candidate in badges_locked_on_board:
                return candidate

    def _remove_star_from_board(self, badges_locked_on_board: list[str]):
